...
Notice that we use a new syntax using the keyword `virtual` to get the reference for interface. Doing so allows us to directly set values to the interface as if the interface is an object.
